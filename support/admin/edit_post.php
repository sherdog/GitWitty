...
{
	$tmpvar['id'] = hesk_isNumber($_REQUEST['reply'],$hesklang['id_not_valid']);
	$sql = "SELECT * FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."replies` WHERE `id`=".hesk_dbEscape($tmpvar['id'])." AND `replyto`='".hesk_dbEscape($ticket['id'])."' LIMIT 1";
	$result = hesk_dbQuery($sql);
	if (hesk_dbNumRows($result) != 1)
    {
...
if (isset($_POST['save']))
{
    ...
    if ($is_reply)
    {
    	...
    	$sql = "UPDATE `".hesk_dbEscape($hesk_settings['db_pfix'])."replies` SET `message`='".hesk_dbEscape($tmpvar['message'])."' WHERE `id`=".hesk_dbEscape($tmpvar['id'])." AND `replyto`='".hesk_dbEscape($ticket['id'])."' LIMIT 1";
    }
    else
    {
