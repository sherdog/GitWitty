/*
@file       inputtree.js
@brief      This is a variant of the mootree.js that add the possibility to use the node to input fields.
	         In fact it allows to add innerHTML before and after the text node.
	         In addition, it allows to use other text class name.
@version    1.2.4
...
Changes:
	...
	rev.16:
	- onClick events added to InputTreeControl and MooTreeNode
	- nodes can now have id's - <InputTreeControl.get> method can be used to find a node with a given id
...
var InputTreeControl = new Class({
	...
	initialize: function(config, options) {
		...
		this.root = new InputTreeNode(options); // create the root node of this tree control

		this.index = new Object();            // used by the get() method

		this.enabled = true;                  // enable visual updates of the control

		this.imgDir = config.imgDir || '';           // Directory of the themes or addition images (icons)
	...
	Parameters:
		options - an object containing the same options available to the <InputTreeNode> constructor.

	Returns:
		A new <InputTreeNode> instance.
	*/
	...
	Property: select
		...
		This is called by <InputTreeNode> when a node is selected (e.g. by clicking it's title with the mouse).

	Parameters:
		node - the <InputTreeNode> object to select.
	*/
	...
	*/
	...
	Note:
		Node id can be assigned via the <InputTreeNode> constructor, e.g. using the <InputTreeNode.insert> method.
	*/
	...
	Parameters:
		...
		parentNode - optional, a <InputTreeNode> object under which to import the specified ul element. Defaults to the root node of the parent control.
	...
	Note:
		...
		CSS-style comments inside a-tags are parsed, and treated as arguments for <InputTreeNode> constructor,
	...
	*/
	...
	Property: disable
		...
		When you're done making changes, call <InputTreeControl.enable> to turn on visual updates
	...
	*/
	...
	/*
	Property: enable
		Enables visual updates again after a call to <InputTreeControl.disable>
	*/
...
/*
Class: InputTreeNode
	This class implements the functionality of a single node in a <InputTreeControl>.

Note:
	You should not manually create objects of this class -- rather, you should use
	<InputTreeControl.insert> to create nodes in the root of the tree, and then use
	the similar function <InputTreeNode.insert> to create subnodes.

	Both insert methods have a similar syntax, and both return the newly created
	<InputTreeNode> object.
...
Options:
	...
	id - string, optional - if specified, must be a unique node identifier. Nodes with id can be retrieved using the <InputTreeControl.get> method.
...
var InputTreeNode = new Class({

	initialize: function(options) {

		this.toolTips = options.toolTips || null;     // Tooltips text to display
		if ( this.toolTips) {
		   // the text displayed by this node with Tooltips
		   this.text = '<label id="' + options.id + '-lbl" for="' + options.id +'"'
		             + ' class="hasTip" title="'+options.text+'::' + this.toolTips + '">'
		             + options.text + '</label>';
		}
	   else {
		...
		this.nodes = new Array();       // subnodes nested beneath this node (InputTreeNode objects)
		this.parent = null;             // this node's parent node (another InputTreeNode object)
		...
		this.beforeText = options.beforeText || null;   // inner HTML to include before the text node
		this.afterText = options.afterText || null;     // inner HTML to include AFTER the text node
		this.textClass = options.textClass || 'mooTree_text2win'; // class style of the text
		...
		this.div = {
			...
			beforeText: this.beforeText ? new Element('div').addClass('mooTree_beforeText') : '',
		...
		this.div.main.adopt(this.div.beforeText);
		...
		this.div.icon._node = this.div.text._node = this.div.beforeText._node = this;
		this.div.icon.onclick = this.div.icon.ondblclick = this.div.beforeText.onclick = this.div.beforeText.ondblclick = this.div.text.onclick       = this.div.text.ondblclick = function() {
	...
	*/
	...
	insert: function(options) {
		...
		var node = new InputTreeNode(options);
	...
	/*
	Property: remove
		Removes this node, and all of it's child nodes. If you want to remove all the childnodes without removing the node itself, use <InputTreeNode.clear>
	*/
	...
	Property: clear
		...
		To remove all nodes including this one, use <InputTreeNode.remove>
	*/
	...
	*/
	...
	update: function(recursive, invalidated) {
		...
		if (draw) {
			...
			if (this.beforeText) {
			   if ( this.div.beforeText.innerHTML.length <= 0) {
			      this.div.beforeText.innerHTML = this.beforeText;
			   }
			}
			if (this.afterText)  x.innerHTML = x.innerHTML + this.afterText;
	...
	*/
	...
	getImg: function(name, div) {
		...
		div.addClass('mooTree_img2win');

		// if a blank image was requested, simply return it now:
		if (name == '') return div;

		var imgDir = this.control.imgDir;
		...
		div.style.backgroundImage = 'url(' + imgDir + img + ')';
		div.style.backgroundPosition = '-' + (i*22) + 'px 0px';
	...
	*/
	...
	/*
	Property: select
		Called by <InputTreeControl> when the selection changes.
		You should not manually call this method - to set the selection, use the <InputTreeControl.select> method.
	*/
