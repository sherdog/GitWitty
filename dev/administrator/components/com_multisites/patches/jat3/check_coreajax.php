...
//------------ jms2win_checkJAT3CoreAjax ---------------
/**
 * check if following lines are present:
 * - if ( defined( 'MULTISITES_ID') ...
 *   is present
 */
function jms2win_checkJAT3CoreAjax( $model, $file)
{
   ...
   if ( !$wrapperIsPresent) {
      ...
      $result .= '|[ACTION]';
      $result .= '|Replace 1 line by 6 lines to compute the appropriate slave site params.ini.';
   }
...
//------------ jms2win_actionJAT3CoreAjax ---------------
/**
 * @brief Install the patch
 */
function jms2win_actionJAT3CoreAjax( $model, $file)
{
   ...
   $patchStr = jms2win_loadPatch( '..' .DS. 'jat3' .DS. 'patch_coreajax.php');
   if ( $patchStr === false) {
      return false;
   }
