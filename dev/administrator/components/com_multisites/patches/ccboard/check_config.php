 ...
 * @brief      Check if the CCBoard configuration wrapper is present.
 * @version    1.2.26
...
//------------ jms2win_checkCCBoardCfgWrapper ---------------
/**
 * check if following lines are present:
 * - if ( defined( 'MULTISITES_ID') ...
 *   is present
 */
function jms2win_checkCCBoardCfgWrapper( $model, $file)
{
   ...
   if ( !$wrapperIsPresent) {
	   $rc = '[NOK]';
      $result .= JText::_( 'The configuration wrapper is not present in the MASTER ccboard-config.php file.');
      $result .= '|[ACTION]';
...
//------------ jms2win_actionCCBoardCfgWrapper ---------------
/**
 * @brief Install the patch
 */
function jms2win_actionCCBoardCfgWrapper( $model, $file)
{
   ...
   $patchStr = jms2win_loadPatch( '..' .DS. 'ccboard' .DS. 'patch_config.php');
   if ( $patchStr === false) {
      return false;
   }
...
//_jms2win_end
   ...
   */
   ...
   $p1 = strpos( $content, 'ccboardConfig');
   if ( $p1 === false) {
      return false;
   }
