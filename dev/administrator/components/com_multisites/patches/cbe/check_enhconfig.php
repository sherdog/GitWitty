...
//------------ jms2win_checkCBECfgWrapper ---------------
/**
 * check if following lines are present:
 * - if ( defined( 'MULTISITES_ID') ...
 *   is present
 */
function jms2win_checkCBECfgWrapperEnhanced( $model, $file)
{
   ...
   if ( !$wrapperIsPresent) {
	   $rc = '[NOK]';
      $result .= JText::_( 'The configuration wrapper is not present in the MASTER CBE enhanced_config.php file.');
      $result .= '|[ACTION]';
...
//------------ jms2win_actionCBECfgWrapperEnhanced ---------------
/**
 * @brief Install the patch
 */
function jms2win_actionCBECfgWrapperEnhanced( $model, $file)
{
   ...
   $patchStr = jms2win_loadPatch( '..' .DS. 'cbe' .DS. 'patch_enhconfig.php');
   if ( $patchStr === false) {
      return false;
   }
...
//_jms2win_end
   ...
   */
   ...
   $p1 = strpos( $content, '$enhanced_Config');
   if ( $p1 === false) {
      return false;
   }
