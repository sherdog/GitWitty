 ...
 * - V1.2.54 19-FEB-2011: Remove the fix introduced by Joomla in version 1.5.16 that create duplicate session
 *                        and that forbid have a Single Sign-In.
 */
      ...
      if ($pos === false) {
         ...
         if ( $sessions_fork === false) {}
            ...
            if (($posCreateSess-$sessions_fork) <= 100) {
               // if '_jms2win_fix_j1_5_22_' is present
...
//_jms2win_end
   ...
   */
   ...
   // PLogin: Search begin statement: "function login"
   $isForkPresent = false;
   ...
   if ( $pb === false) {}
      ...
      for ( $popencomment = $pa-2; $popencomment>0; $popencomment--) {
         ...
         if ( $c == "\n" || $c == "\r" || $c == "\t" || $c == " ") {}
            ...
            // If wait for "/"*
               ...
               if ( $c == '/') {
                  // Success
                  // Update the pa position to include the "/*" comment
                  // Pa: Go to Begin of line
      ...
      if ( $pe === false) {}
      // If Joomla has fixed the bug introduced in Joomla 1.5.16 or higher
      else if ( ($pe-$pc) < 100) {
         // pf: Search for end of line of createSession
         for ( $pf=$pe; $content[$pf] != "\n"; $pf++);
         $pf++;
      

         // If Jms fix 1.5.22 is already present
         $p1522 = strpos( $content, '_jms2win_fix_j1_5_22_', $plogin);
         if ( $p1522 === false) {
            // Check if immediately followed by a "closed" comment
            $pclosecomment = strpos( $content, '*/', $pf);
            if ( $pclosecomment === false) {}
            else {
               // Strcom = the string between '\n' and the '*/'
               $strCom = trim( substr( $content, $pf, $pclosecomment-$pf));
               // If there is no string between the '\n' and the '*/'
               if ( empty( $strCom)) {
                  // Then include the '*/' and search for end of line
                  // pf: Search for end of line of the "close" comment
            ...
            // Then do not re-install the patch
            $isForkPresent = false;
         }
      }
      if ( empty( $pf)) { $pf = $pc; }
