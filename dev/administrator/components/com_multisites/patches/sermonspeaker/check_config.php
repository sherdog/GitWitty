 ...
 * @brief      Check if the SermonSpeaker configuration wrapper is present.
 * @version    1.2.12
...
//------------ jms2win_checkSermonCfgWrapper ---------------
/**
 * check if following lines are present:
 * - if ( defined( 'MULTISITES_ID') ...
 *   is present
 */
function jms2win_checkSermonCfgWrapper( $model, $file)
{
   ...
   if ( !$wrapperIsPresent) {
	   $rc = '[NOK]';
      $result .= JText::_( 'The configuration wrapper is not present in the MASTER SemonSpeaker config.sermonspeaker.php file.');
      $result .= '|[ACTION]';
...
//------------ jms2win_actionSermonCfgWrapper ---------------
/**
 * @brief Install the patch
 */
function jms2win_actionSermonCfgWrapper( $model, $file)
{
   ...
   $patchStr = jms2win_loadPatch( '..' .DS. 'sermonspeaker' .DS. 'patch_config.php');
   if ( $patchStr === false) {
      return false;
   }
...
if ( defined( 'MULTISITES_ID')
  && file_exists( dirname(__FILE__) .DS. 'config.sermonspeaker.' .MULTISITES_ID. '.php')) {
   require_once(  dirname(__FILE__) .DS. 'config.sermonspeaker.' .MULTISITES_ID. '.php');
} else  {
//_jms2win_end
      class sermonConfig {
      {
...
//_jms2win_end
   ...
   */
   ...
   // p1: Search for "sermonConfig"
   $p1 = strpos( $content, 'sermonConfig');
   if ( $p1 === false) {
      return false;
   }
