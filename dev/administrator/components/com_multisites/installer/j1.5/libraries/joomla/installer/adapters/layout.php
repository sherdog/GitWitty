...
/**
 * @file       layout.php
 * @brief      Layout adapter for the JMS front-end layouts (templates).
 *
 * @version    1.2.20
 ...
 * @remark     Is inpired from the joomla "/libraries/joomla/installer/adapter/template.php".
 * @par History:
 ...
 * @since		1.5
 */
class MultisitesInstallerLayout extends JObject
{
	/**
	 * Constructor
	 *
	...
	function install()
	{
		...
		if ($cname = $root->attributes('client')) {
			...
			$basePath = $client->path .DS. 'components' .DS. 'com_multisites';
			$clientId = $client->id;
		} else {
			// No client attribute was found so we assume the site as the client
			$cname = 'site';
			$basePath = JPATH_SITE .DS. 'components' .DS. 'com_multisites';
			$clientId = 0;
		}
	...
	function uninstall( $name, $clientId )
	{
		...
		$this->parent->setPath('extension_root', $client->path .DS. 'components' .DS. 'com_multisites' .DS.'templates'.DS.$name);
