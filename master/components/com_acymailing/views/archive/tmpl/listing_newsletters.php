		...
		<tbody>
			...
			?>
				<tr class="<?php echo "sectiontableentry$k".$this->values->suffix; ?>">
					...
					<td>
						<a <?php if($this->config->get('open_popup')){ echo 'class="modal" rel="{handler: \'iframe\', size: {x: 750, y: 550}}"'; } ?> href="<?php echo acymailing::completeLink('archive&task=view&listid='.$this->list->listid.'-'.$this->list->alias.'&mailid='.$row->mailid.'-'.strip_tags($row->alias).$this->item,(bool)$this->config->get('open_popup')); ?>">
						...
						<?php if($this->access->frontEndManament){
							...
							<?php } if(!empty($row->senddate)){ ?>
							<span class="acystatsbutton"><a class="modal" rel="{handler: 'iframe', size: {x: 800, y: 590}}" href="<?php echo acymailing::completeLink('newsletter&task=stats&mailid='.$row->mailid.'&listid='.$this->list->listid,true); ?>"><img src="<?php echo ACYMAILING_IMAGES; ?>icons/icon-16-stats.png"/></a></span>
							<?php } ?>
						<?php }?>
					</td>
					<?php if($this->values->show_senddate) { ?>
