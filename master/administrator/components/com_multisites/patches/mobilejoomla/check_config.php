 ...
 * @brief      Check if the Mobile Joomla configuration wrapper is present.
 * @version    1.2.45
 ...
 * @par History:
 * - V1.2.45 05-OCT-2010: Initial version
 */

defined('JPATH_MUTLISITES_COMPONENT') or die( 'Restricted access' );

//------------ jms2win_checkAlphaContentWrapper ---------------
/**
 * check if following lines are present:
 * - if ( defined( 'MULTISITES_ID') ...
 *   is present
 */
function jms2win_checkMobJoomCfgWrapper( $model, $file)
{
   ...
   if ( !$wrapperIsPresent) {
	   $rc = '[NOK]';
      $result .= JText::_( 'The routing wrapper is not present in the MASTER mobile joomla config.php file.');
      $result .= '|[ACTION]';
...
//------------ jms2win_actionAlphaContentWrapper ---------------
/**
 * @brief Install the patch
 */
function jms2win_actionMobJoomCfgWrapper( $model, $file)
{
   ...
   $patchStr = jms2win_loadPatch( '..' .DS. 'mobilejoomla' .DS. 'patch_config.php');
   if ( $patchStr === false) {
      return false;
   }
...
//	$filename = JPATH_ROOT.DS.$file;
   ...
   */
   ...
   /* ....\n
      ...
      Produce
      begin -> p0 + INSERT PATCH + p6 + "}" + p6 -> end
      
    */
   
   // p1: Search for "class"
   $p1 = strpos( $content, 'MobileJoomla_Settings');
   if ( $p1 === false) {
      return false;
   }
