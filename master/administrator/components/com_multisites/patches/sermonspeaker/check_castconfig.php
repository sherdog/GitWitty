<?php
/**
 * @file       check_castconfig.php
 * @brief      Check if the wrapper is present in "sermoncastconfig.sermonspeaker.php" configuration file.
 * @version    1.2.12
...
//------------ jms2win_checkSermonCastCfgWrapper ---------------
/**
 * check if following lines are present:
 * - if ( defined( 'MULTISITES_ID') ...
 *   is present
 */
function jms2win_checkSermonCastCfgWrapper( $model, $file)
{
   ...
   if ( !$wrapperIsPresent) {
	   $rc = '[NOK]';
      $result .= JText::_( 'The configuration wrapper is not present in the MASTER SemonSpeaker sermoncastconfig.sermonspeaker.php file.');
      $result .= '|[ACTION]';
...
//------------ jms2win_actionSermonCastCfgWrapper ---------------
/**
 * @brief Install the patch
 */
function jms2win_actionSermonCastCfgWrapper( $model, $file)
{
   ...
   $patchStr = jms2win_loadPatch( '..' .DS. 'sermonspeaker' .DS. 'patch_castconfig.php');
   if ( $patchStr === false) {
      return false;
   }
...
//_jms2win_end
   ...
   */
   ...
   // p1: Search for "sermonCastConfig"
   $p1 = strpos( $content, 'sermonCastConfig');
   if ( $p1 === false) {
      return false;
   }
