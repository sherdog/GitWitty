<?php
/**
 * @file       check_ueconfig.php
 * @brief      Check if the Community Builder Enhanced configuration wrapper is present.
 * @version    1.2.52
 ...
 * @par History:
 * - V1.2.52 13-JAN-2011: Initial version
 */

defined('JPATH_MUTLISITES_COMPONENT') or die( 'Restricted access' );

//------------ jms2win_checkCBECfgWrapperUE ---------------
/**
 * check if following lines are present:
 * - if ( defined( 'MULTISITES_ID') ...
 *   is present
 */
function jms2win_checkCBECfgWrapperUE( $model, $file)
{
   ...
   if ( !$wrapperIsPresent) {
	   $rc = '[NOK]';
      $result .= JText::_( 'The configuration wrapper is not present in the MASTER CBE ue_config.php file.');
      $result .= '|[ACTION]';
...
//------------ jms2win_actionCBECfgWrapperUE ---------------
/**
 * @brief Install the patch
 */
function jms2win_actionCBECfgWrapperUE( $model, $file)
{
   ...
   $patchStr = jms2win_loadPatch( '..' .DS. 'cbe' .DS. 'patch_ueconfig.php');
   if ( $patchStr === false) {
      return false;
   }
...
//	$filename = JPATH_ROOT.DS.$file;
   ...
   /*
      ...
      ===========
		...
		........
//_jms2win_begin
if ( defined( 'MULTISITES_ID') && file_exists( dirname( __FILE__).DS.'ue_config_' . MULTISITES_ID . '.php')) {
...
//_jms2win_end
   ...
   */
   ...
   // p1: Search for "$ueConfig"
   $p1 = strpos( $content, '$ueConfig');
   if ( $p1 === false) {
      return false;
   }
