...
//Include  cPanel class files
@include_once( JPATH_ROOT.DIRECTORY_SEPARATOR.'administrator'.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_multisites' .DIRECTORY_SEPARATOR. 'classes' .DIRECTORY_SEPARATOR. 'cpanel.class.php');
@include_once( JPATH_ROOT.DIRECTORY_SEPARATOR.'administrator'.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_multisites' .DIRECTORY_SEPARATOR. 'classes' .DIRECTORY_SEPARATOR. 'ecurl.class.php');
...
// ===========================================================
   ...
   //------------ createDatabase ---------------
	...
	function createDatabase(& $db, $DBname, $DButfSupport)
	{
		...
		$cpanel = new cPanelAPI($cpanel_host, $cpanel_user, $cpanel_password, $cpanel_skin);
      	...
      	$errors = array();
		$name = str_replace("gitwitty_", '', $DBname);

      //we are removing the old functions here and just replacing with the cPanel db creation since we don't have 
	  //root login information for this database.
		$createArray =  $cpanel->mysql_easy_create($name);
   ...
   //------------ _getDBUserName ---------------
	...
	function _getDBUserName( $toConfig)
	{
	   ...
	   if ( empty( $ip)) {
	      $user_name  = $toConfig->getValue( 'config.user');
	   }
	   else {
	      $user_name  = $toConfig->getValue( 'config.user');
	   }
   ...
   //------------ _createUser ---------------
	...
	function _createUser( & $db, $toConfig, $table_name = null)
	{
	   ...
	   if ( empty( $table_name)) {
	      $table_name = $toConfig->getValue( 'config.db');
	  ...
	  $cpanel->mysql_add_user_to_db('gitwitty_dbadmin', $table_name);
	  // exit('made it to 2678 adding user ' . $user_name . ' to db: ' . $table_name);
