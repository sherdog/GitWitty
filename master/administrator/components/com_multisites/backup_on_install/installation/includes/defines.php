...
// Cross-check that admin patch is present to avoid infinite loop
$tmpStr = file_get_contents( JPATH_ADMINISTRATOR.DS.'includes'.DS.'defines.php');
if ( $tmpStr === false) {
   unset( $tmpStr);
}
else {
   $pos = strpos( $tmpStr, 'defines_multisites.php');
   unset( $tmpStr);
   if ($pos === false) {
      unset( $pos);
   }
