...
?>
<p>You can change your template at any time, just select the template below and then click on the "default" button in the submenu</p>
<style>
.templateBox { float:left; width:250px; margin:5px; padding:10px; border:1px solid #CCC; }
.templateBoxIMG { height:150px; width:206px; overflow:hidden; margin:0px auto; }
.templateBoxBottom { font-family:Verdana, Geneva, sans-serif; font-size:11px; font-weight:bold; }
.highlighted { background-color:#CCC; }
.templateChooseAnchor2 { margin:10px 0px; text-align:center; display:block; }
</style>
<script>
jQuery(document).ready(function(){
	jQuery('.templateChooseAnchor').click(function() {
		jQuery('.highlighted').removeClass('highlighted');
		jQuery(this).parent('div').parent('div').addClass('highlighted');
	});
	jQuery('.templateChooseAnchor2').click(function() {
		jQuery('.highlighted').removeClass('highlighted');
...
?>
<?php $img_path = ($client->id == 1 ? JURI::root().'administrator' : $mainframe->getSiteURL() ).'/templates/'.$row->directory.'/template_thumbnail.png'; ?>


    <div class="templateBox" id="template_<?=$i?>">
        <div class="templateBoxIMG">
         	<a href="javascript:void(0);" class="templateChooseAnchor" onclick="document.adminForm.cb<?php echo $i;?>.checked = true; document.adminForm.boxchecked.value='1'; ">
            <img border="0" src="<?php echo $img_path; ?>" name="imagelib" alt="<?php echo JText::_( 'No preview available' ); ?>" width="206" height="145" />
        	</a>
        </div>
    	
        <a href="javascript:void(0);" class="templateChooseAnchor2" onclick="document.adminForm.cb<?php echo $i;?>.checked = true; document.adminForm.boxchecked.value='1'; ">
        <img src="http://master.gitwitty.com/images/btnUseThisTemplate.png" border="0" />
        </a>
        <span style="display:none;"><input type="radio" id="cb<?php echo $i;?>" checked="" name="cid[]" value="<?php echo $row->directory; ?>" onclick="isChecked(this.checked);" /></span>
    </div>
