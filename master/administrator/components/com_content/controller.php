...
class ContentController extends JController
{
	...
	*/
	function viewContent()
	{
		...
		/*
		$mq = "SELECT name, id FROM #__menu WHERE published=1 AND type='component'";
		$db->setQuery($mq);
		$menuOptions = $db->loadResultArray();
		
		
		foreach($menuOptions as $key=>$val) {
			$mOptions[] = JHTML::_('select.option', $key, $val);
		}
		print_r($mOptions);
		$menuSelect = JHTML::_('select.genericlist', $mOptions, 'menu', 'class="inputbox"', 'value', 'text', $default);
		echo $menuSelect;
		*/
