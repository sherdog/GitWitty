...
function insertMenuItem( &$template )
{
	...
	//added if no items were selected... we erase it from the menu.
		...
		$item->params .= implode( "\n", $menuparams );
		//delete from db altogether
		
		
		
		
		$query = 'DELETE FROM #__menu WHERE alias = "'.$item->alias.'"';
		$db->setQuery( $query );
		$db->query(); 
...
function renderHTML( &$template )
{
	...
	$width = '100%';
	$elements = array();
	$elements[] = el('', '<input type="hidden" id="alias" value="'.$item->alias.'">');
	$elements[] = el(
		'Title',
	...
	$elements[] = el(
		'Alias',
		'<input class="inputbox" type="text" disabled="true" name="alias" style=width:'.$width.';" maxlength="255" value="'.$item->alias.'" />'
	);
	$elements[] = el(
		'Published',
...
function getMenuItems( $name, $width = '100%' )
{
	...
	$options = array();
	//$options[]	= JHTML::_( 'select.option', 'remove::-1', '[ REMOVE FROM MENU ]' );
...
?>
	<form action="<?php echo $uri->toString(); ?>" method="post" name="adminForm" id="adminForm">
		...
		<fieldset>
			<div style="float: right" class="mc-toolbar">
				<ul>
                	
                	<li class="button">
                    <a rel="" class="toolbar" onclick="document.getElementById('adminForm').submit();" href="javascript://" title="<?php echo JText::_('Add') ?>"><?php echo JText::_('Add') ?></a>
                    </li>
                    <li class="button">
                    <a rel="" class="toolbar" onclick="window.parent.document.getElementById('sbox-window').close();" href="javascript://" title="<?php echo JText::_('Cancel') ?>"><?php echo JText::_('Cancel') ?></a>
                    </li>
                </ul>
		...
		<fieldset>
           ...
           <script type="text/javascript" src="http://www.google.com/jsapi"></script>
			...
			$(document).ready(function() {
				$('#removeMenuItem').click(function(e) {
					e.preventDefault();
					$.ajax({
						url: 'removemenu.php',
						type: 'post',
