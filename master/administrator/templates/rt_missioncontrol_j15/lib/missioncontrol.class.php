...
class MissionControl extends RTCore {
	...
	function displayUserInfo() {
		...
		if (intval($unread) > 0)
			$messages = $this->_addListItem($inboxLink.'(<span class="unread-messages">'.$unread.'</span>) '.JText::_('MESSAGES').'</a>','special');
		else
	...
	function displayStatus() {
		...
		if ($disabled) 
			$output[] = '<span class="preview"><a>'.JText::_('VIEW_SITE').'</a></span>';
		else 
        ...
        // display editors
        $output[] = array($this->_renderEditorSelect(), 'dropdown');
		*/
    ...
    function _renderEditorSelect() {
        ...
        $editor_script = "\n
			var updateEditor = function(){
				...
				}).send('index.php', 'option=com_users&task=save&id=". $user->get('id') ."&sendEmail=0&". JUtility::getToken()."=1&username=" . $user->get('username') ."&params[editor]='+editor.value+'&tmpl=component');
			};
