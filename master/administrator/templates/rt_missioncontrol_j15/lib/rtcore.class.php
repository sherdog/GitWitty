...
class RTCore
{
    ...
    function getMooScriptVersion($filename) {
        ...
        if (JPluginHelper::isEnabled('system', 'mtupgrade') || $moo_override) {
    ...
    function addOverrideStyles()
    {
        ...
        $override_url = $this->templateUrl . '/overrides/' . $option . DS . $override;
    ...
    function _initToolbar()
    {
        ...
        //if it's not, then continue like always
        if(!$bar->_bar) {
        	if($this->_initKoowaToolbar()) {
        		return;
        	}
        }
    ...
    function _initKoowaToolbar()
    {
    	if(class_exists('KFactory')) {
    		$bar = & JToolBar::getInstance('toolbar');
    		
    		$option		= str_replace('com_', '', KRequest::get('get.option', 'cmd'));
    		$view		= KRequest::get('get.view', 'cmd');
    		$format		= KRequest::get('get.format', 'cmd', 'html');
    		try {
	    		$identifier	= KFactory::get('admin::com.'.$option.'.view.'.$view.'.'.$format)->getToolbar();
	    		$toolbar	= KFactory::get($identifier);
	    		if(method_exists($toolbar, 'getName')) {
	    			$name		= 'toolbar-'.$toolbar->getName();
	    		}
	    		$buttons = $toolbar->get('_buttons');
		        ...
		        foreach ($buttons as $button) {
		            ...
		            if (strtolower($button->_options->text) == 'help' || strtolower($button->_options->text) == 'about') {
		                $newhelp[] = $button;
		            } elseif (strtolower($button->_options->text) == 'unarchive' or
		                      strtolower($button->_options->text) == 'archive' or
		                      strtolower($button->_options->text) == 'enable' or
		                      strtolower($button->_options->text) == 'disable' or
		                      //strtolower($button->_options->text) == 'move' or
		                      //strtolower($button->_options->text) == 'copy' or
		                      strtolower($button->_options->text) == 'trash') {
		                $actions[] = $button;
		            } else if (strtolower($button->_options->text) == 'new' or
		                       strtolower($button->_options->text) == 'apply' or
		                       strtolower($button->_options->text) == 'save') {
		                $first[] = $button;
		            } else {
		                $newbar[] = $button;
		            }
    ...
    function _addListItem($item, $class = null, $link = null)
    {
    ...
    function _getTools()
    {
     ...
     //   $canConfig = $user->authorize('com_config', 'manage');
      ...
      //      $tools = array();
      //      if ($canConfig) {
      //          $tools[] = $this->_addListItem(JText::_('Read Messages'), 'messages', 'index.php?option=com_messages');
       //         $tools[] = $this->_addListItem(JText::_('Write Message'), 'messages', 'index.php?option=com_messages&task=add');
      //      }
       //     if ($canMassMail) {
       //         $tools[] = $this->_addListItem(JText::_('Mass Mail'), 'massmail', 'index.php?option=com_massmail');
       //     }
        //    if ($canCheckin) {
        //        $tools[] = $this->_addListItem(JText::_('Global Checkin'), 'checkin', 'index.php?option=com_checkin');
       //     }
         //   $tools[] = $this->_addListItem(JText::_('Clean Cache'), 'config', 'index.php?option=com_cache');
