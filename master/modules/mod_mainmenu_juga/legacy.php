...
*/
function jugaGetMenuLink($mitem, $level = 0, & $params, $open = null)
{
	global $Itemid;
	$txt = '';
	...
	if (!$current_itemid) {
		...
		if ($current_itemid == $mitem->id) {
			...
			if ($params->get('activate_parent') && isset ($open) && in_array($mitem->id, $open)) {
				...
				if ($mitem->type == 'url' && jugaItemidContained($mitem->link, $current_itemid)) {
					$id = 'id="active_menu' . $params->get('class_sfx') . '"';
				} else {
					$id = '';
				}
...
*/
function jugaShowVIMenu(& $params)
{
	...
	jugaRecurseVIMenu(0, 0, $children, $open, $indents, $params);
}
...
* hierarchial menu
*/
function jugaRecurseVIMenu($id, $level, & $children, & $open, & $indents, & $params)
{
	if (@ $children[$id]) {
		...
		foreach ($children[$id] as $row) {
			...
			echo jugaGetMenuLink($row, $level, $params, $open);

			// show menu with menu expanded - submenus visible
			if (!$params->get('expand_menu')) {
				if (in_array($row->id, $open)) {
					jugaRecurseVIMenu($row->id, $level +1, $children, $open, $indents, $params);
				}
			} else {
				jugaRecurseVIMenu($row->id, $level +1, $children, $open, $indents, $params);
			}
...
*/
function jugaShowHFMenu(& $params, $style = 0)
{
	$menu = & JSite::getMenu();
	$user = & JFactory::getUser();
	...
	if(is_array($rows) && count($rows)) {
		foreach ($rows as $row)
		{
			if ($row->access <= $user->get('aid', 0)) {
				$links[] = jugaGetMenuLink($row, 0, $params);
			}
		}
	}
...
*/
	...
	function jugaItemidContained($link, $Itemid)
	{
