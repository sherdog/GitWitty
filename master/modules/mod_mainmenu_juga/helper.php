...
require_once($absolute_path.'/administrator/components/com_juga/juga.class.php');

/**
 * mod_mainmenu_juga Helper class
 *
 * @static
 * @package		Joomla
 * @subpackage	Menus
 * @since		1.5
 */
class jugaMainMenuHelper
{
	function buildXML(&$params)
	{
		$menu = new jugaMenuTree($params);
		$items = &JSite::getMenu();
		...
		// foreach row, 
		// parse link to variables
		// pass variables to jugaRightsCheck
		// if ok, save
		// if not, blank
		$jugaRows = array();
		...
		if ($rows) { foreach ($rows as $r) {
			unset($output);			
			unset($jugaUrl);
			$jugaUrl = parse_url($r->link);
			parse_str($jugaUrl['query'], $output);

			// **********************************************************************
			// check user's rights to be on selected page with option, section, task
			$details["user"] 	= $user; 
			$details["option"] 	= $output["option"]; 
			$details["section"] = $output["section"]; 
			$details["view"] 	= $output["view"];
			$details["task"] 	= $output["task"];
			$details["id"]		= $output["id"];
			$details["site"]	= "site";
	...
	function &getXML($type, &$params, $decorator)
	{
		...
		if (!isset($xmls[$type])) {
			$cache =& JFactory::getCache('mod_mainmenu_juga');
			$string = $cache->call(array('jugaMainMenuHelper', 'buildXML'), $params);
			$xmls[$type] = $string;
		}
	...
	function render(&$params, $callback)
	{
		switch ( $params->get( 'menu_style', 'list' ) )
		{
			case 'list_flat' :
				...
				jugaShowHFMenu($params, 1);
				break;

			case 'horiz_flat' :
				// Include the legacy library file
				require_once(dirname(__FILE__).DS.'legacy.php');
				jugaShowHFMenu($params, 0);
				break;

			case 'vert_indent' :
				// Include the legacy library file
				require_once(dirname(__FILE__).DS.'legacy.php');
				jugaShowVIMenu($params);
				break;

			default :
				// Include the new menu class
				$xml = jugaMainMenuHelper::getXML($params->get('menutype'), $params, $callback);
				if ($xml) {
 ...
 * @since		1.5
 */
class jugaMenuTree extends JTree
{
	/**
	...
	{
		$this->_params		=& $params;
		$this->_root		= new jugaMenuNode(0, 'ROOT');
	...
	function addNode($item)
	{
		...
		$node = new jugaMenuNode($item->id, $item->name, $item->access, $data);
 ...
 * @since		1.5
 */
class jugaMenuNode extends JNode
{
	/**
	 * Node Title
	 */
