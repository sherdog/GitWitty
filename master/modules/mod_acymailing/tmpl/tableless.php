			...
			<div class="acymailing_form">
					<?php foreach($fieldsToDisplay as $oneField){
						...
						if($oneField == 'name' AND empty($extraFields[$oneField])){
							if($displayOutside) echo '<label for="user_name_'.$formName.'">'.$nameCaption.'</label>'; ?>
								<span><input id="user_name_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo 'disabled="disabled" ';  if(!$displayOutside){ ?> onfocus="if(this.value == '<?php echo $nameCaption;?>') this.value = '';" onblur="if(this.value=='') this.value='<?php echo $nameCaption?>';"<?php } ?> class="inputbox" type="text" name="user[name]" size="<?php echo $params->get('fieldsize',15)?>" value="<?php if(!empty($identifiedUser->userid)) echo $identifiedUser->name; elseif(!$displayOutside) echo $nameCaption; ?>" /></span>
							</p> <?php
							...
							if($displayOutside) echo '<label for="user_email_'.$formName.'">'.$emailCaption.'</label>'; ?>
								<span><input id="user_email_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo 'disabled="disabled" ';  if(!$displayOutside){ ?> onfocus="if(this.value == '<?php echo $emailCaption;?>') this.value = '';" onblur="if(this.value=='') this.value='<?php echo $emailCaption?>';"<?php } ?> class="inputbox" type="text" name="user[email]" size="<?php echo $params->get('fieldsize',15)?>" value="<?php if(!empty($identifiedUser->userid)) echo $identifiedUser->email; elseif(!$displayOutside) echo $emailCaption; ?>" /></span>
							</p> <?php
						}elseif($oneField == 'html' AND empty($extraFields[$oneField])){
							echo '<label for="user_html_'.$formName.'">'.JText::_('RECEIVE').'</label>';
							echo '<span>'.JHTML::_('select.booleanlist', "user[html]" ,'',isset($identifiedUser->html) ? $identifiedUser->html : 1,JText::_('HTML'),JText::_('JOOMEXT_TEXT'),'user_html_'.$formName).'</span>';
				...
				<?php }
				 if($params->get('showterms',false)){
					...
					?>
					<span><input id="mailingdata_terms_<?php echo $formName; ?>" class="inputbox" type="checkbox" name="terms"/></span>
