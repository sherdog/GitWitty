	...
	tinymce.create('tinymce.themes.AdvancedTheme', {
		...
		init : function(ed, url) {
			...
			t.settings = s = extend({
				...
				theme_advanced_buttons1 : "bold,italic,underline,strikethrough,|,bullist,numlist,|,link,unlink,|,insertUser,insertPicture,insertVideo,insertSmile,|,asDefault,asHTML",
			...
			ed.onSetContent.add(t._bigimgToIMG);
		...
		_bigimgToIMG : function(ed,o){
			var dom = ed.dom;
			each(dom.select('img', o.content), function(ob) {
				var p = dom.getParent(ob,"a");
				if (p && dom.getAttrib(p,"rel")!=""){
					var rel = dom.getAttrib(p,"rel"), href = dom.getAttrib(p,"href"), title = dom.getAttrib(p,"title");
					var src = dom.getAttrib(ob,"src"), style = dom.getAttrib(ob,"style"), alt = dom.getAttrib(ob,"alt"), width=dom.getAttrib(ob,"width"), height=dom.getAttrib(ob,"height");
					
					var q = /\[(.*)\]/.exec(rel);
					if (q!=null){
						var i = dom.create("img",{'alt':alt,'src':src,'original_size':q[1],'original':href,'class':"mceBigImg", 'width':width, 'height':height, 'title':title, 'style':style});
						dom.replace(i,p);					
					}
				}
			});
		...
		_IMGToBigimg : function(ed,o){
			var dom = ed.dom, t=this;
			
			each(dom.select('img', o.node), function(ob) {
				if (dom.getAttrib(ob, 'class')=="mceBigImg"){
					var title = dom.getAttrib(ob,"title");
					var noindex = dom.getAttrib(ob,"noindex");
					if (title=="") title = dom.getAttrib(ob,"alt");
					var a = dom.create("a",{'href':dom.getAttrib(ob,"original"),'rel':(ed.getParam('use_rokbox')?"rokbox":"")+'['+dom.getAttrib(ob,"original_size")+']','title':title,'target':'_blank'});
					var img = dom.create("img",{'width':dom.getAttrib(ob,"width"),'height':dom.getAttrib(ob,"height"),'src':dom.getAttrib(ob,"src"),'style':dom.getAttrib(ob,"style"),'alt':dom.getAttrib(ob,"alt")});
					if (noindex){
						var ni = dom.getParent(ob,"noindex");
						if (ni!=null) noindex = false;
					}
					if (noindex){
						var ni = dom.create("noindex");
						dom.replace(ni,ob);
						dom.add(ni,a);
					} else{
						dom.replace(a,ob);
		...
		_showDialog:function(loadUrl){
			...
			dialog.innerHTML = '<img src="'+ed.baseURI.toAbsolute('themes/advanced/skins/') + ed.settings.skin+'/img/progress.gif">';
			overlay.onclick = function(){removeDialog();}
...
*/
		...
		_mceFile : function(ui, val) {
			...
			ed.windowManager.open({
				url : tinymce.baseURL + '/themes/advanced/file.htm',
				width : 355 + parseInt(ed.getLang('advanced.image_delta_width', 0)),
				height : 175 + parseInt(ed.getLang('advanced.image_delta_height', 0)),
				inline : true,
				overlay:false
			}, {
				theme_url : this.url
		...
		_mceUser : function(ui, val) {
			var ed = this.editor;

			ed.windowManager.open({
				url : tinymce.baseURL + '/themes/advanced/user.htm',
				width : 436 + parseInt(ed.getLang('advanced.image_delta_width', 0)),
				height : 400 + parseInt(ed.getLang('advanced.image_delta_height', 0)),
				inline : true,
				overlay:false
			}, {
				theme_url : this.url
