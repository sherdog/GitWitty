...
//BADE
$carryoutonmenuwithname = $this->params->get('carryoutonmenuwithname'); //"mainmenu"; //
...
//
		...
		//BADE carryoutonmenuwithname now searching for things to use the sub menu for. 
					

		    if ($menutype_title_matches && $carryoutonmenuwithname) {
			$dbquery = 'SELECT * FROM #__menu WHERE name REGEXP %s' ; 
			...
			$dbquery = $dbquery . ' and menutype = "' . $carryoutonmenuwithname . '"' ;
                ...
                if ($menutype_tocreatesubmenu) {
                    ...
                    $new_menuitem_tocreatesubmenu = &$this->createMenuItem(
                        ...
                        $this->defaultComContentParams(),
                        $com_articles->id,
						$menutype_tocreatesubmenu->id, ($menutype_tocreatesubmenu->sublevel + 1)
                    );
		...
		//		end carryoutonmenuwithname	
            ...
            if ($menu_ids and count($menu_ids)){
				...
				JError:: raiseNotice(500, "The Publishing state of related menu item(s) was changed to :" 
				. (($article->state) ? plgSystemAutoMenu_Magic::menustate_published : plgSystemAutoMenu_Magic::menustate_unpublished
) );}
