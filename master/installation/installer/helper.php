...
class JInstallationHelper
{
	...
	function FTPVerify($user, $pass, $root, $host='127.0.0.1', $port='21')
	{
		...
		if ( defined( 'MULTISITES_ID')) {
   		$checkList = array('config_multisites.php');
		}
		...
		$buffer = null;
		if ( defined( 'MULTISITES_ID')) {
   		if ($ftp->read($root.'/config_multisites.php', $buffer) === false) {
   			return JError::raiseError('SOME_ERROR_CODE', 'NORETR');
   		}
		}
		else if ($ftp->read($root.'/libraries/joomla/version.php', $buffer) === false) {
			return JError::raiseError('SOME_ERROR_CODE', 'NORETR');
		}

		// Verify valid root path, part two
		if ( defined( 'MULTISITES_ID')) {
   		$checkValue = file_get_contents(JPATH_ROOT.DS.'multisites'.DS.MULTISITES_ID.DS.'config_multisites.php');
		}
		else {
   		$checkValue = file_get_contents(JPATH_LIBRARIES.DS.'joomla'.DS.'version.php');
		}
