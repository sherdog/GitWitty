...
class ContentController extends JController
{
	...
	*/
	function viewContent()
	{
		...
		//get list of menu items as we are overriding Joomla lack of simpleness.. fucking lamers.
		//recursive menu items.. prepend -- to anything that mayyy be a submenu (has parent :P
		$lists['menu'] = recurseMenu();
	...
	function recurseMenu($parent = 0, $level = 0) {
		
		global $mainframe;
		$menus = array();//just initializing the menus array that we will be returning back to something eh?
		$db = & JFactory::getDBO();
		
		$user =  & JFactory::getUser();
		$indent = "&nbsp;&nbsp;";
		
		$query = "SELECT * FROM #__menu WHERE parent = ".$parent." ORDER BY ordering ASC";
		$db->setQuery($query);
		...
		foreach($items as $item) {
			if($item['parent'] != 0)
			{
				$menus[] = recurseMenu($item['id'], $level++);	
			} else {
				if($level > 0) {
					for($i=0;$i<$level;$i++)  {
						$indentchars += $indent;
					...
					$title =  $indentchars . $item['name'];
					$menus[] = array('id'=>$item['id'], 'title'=>$title);
				}
			}
		}
